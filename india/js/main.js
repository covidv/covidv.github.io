    ...
    $.getJSON('https://api.rootnet.in/covid19-in/contacts', function(result) {
        $.each(result['data']['contacts']['regional'], function(key, value) {
            ...
            if(mobile.indexOf(',') > 0)
                ...
                var data = "<div class='card'> <div class='header'><h6 class='mb-0 text-center' style='color: ywllow'>"+value['loc']+"</h6> </div> <div class='body'><p class='black-text mb-0 text-center'><a href='tel:"+mobile.replace(new RegExp('-', 'g'),"")+"'>"+mobile+"</a></p></div></div>";
